...
def _print_dump(mesh: NavMesh, limit: int = 20) -> None:
    for i, area in enumerate(mesh.areas.values()):
        ...
        print(
            ...
            f"  light=[{','.join(f'{v:.2f}' for v in area.light)}]"
        )
...
def _export_json(mesh: NavMesh, path: str) -> None:
    ...
    out = {
        ...
        "ladders": {
            str(ldr.id): {
                "top": ldr.top.to_list(),
                "bottom": ldr.bottom.to_list(),
                "width": round(ldr.width, 2),
                "length": round(ldr.length, 2),
                "connected_areas": {
                    "top_forward": ldr.top_forward_area_id,
                    "top_left": ldr.top_left_area_id,
                    "top_right": ldr.top_right_area_id,
                    "top_behind": ldr.top_behind_area_id,
                    "bottom": ldr.bottom_area_id,
                },
            }
            for ldr in mesh.ladders.values()
        },
    }
